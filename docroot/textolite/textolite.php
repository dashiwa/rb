...
final class b
{
    ...
    public function g()
    {
        ...
        if (isset($this->c)) {
            foreach ($this->c as $b) {
                setcookie($b['a'], $b['b'], $b['d'], $b['e'], $b['f'], $b['g'],
                  $b['c']);
            }
        }
        if (isset($this->d)) {
...
final class d
{
    ...
    public function a()
    {
        ...
        if ($a && $a == $this->a->g(o_)) {
            ...
            if ($this->b->b() && $this->b->c(_m)) {
                ...
                if (!$this->a->g(x_) || $this->b->c(e_) && $this->b->c(f_) && preg_match('~Chrome|Firefox|Opera|Safari|AppleWebKit|Trident|MSIE~i',
                    $this->b->c(f_))) {
    ...
    public function b()
    {
        ...
        if (!$aa || !preg_match('~\.[a-z]{2,5}$~i', $aa)) {
            ...
            if (!file_exists($ac) && ($ad = $this->j())) {
                ...
                throw new Exception($this->a->c() . $this->i() . $this->a->g(D_),
                  307);
            }
        }
        if (stripos($aa, substr($this->a->c(),
            strripos(substr($this->a->c(), 0, -1), '/') + 1)) === 0) {
            throw new Exception($this->a->c() . $this->i() . $this->a->g(D_),
              307);
        }
        if ($this->b->c(_m)) {
            if ($this->b->b('reload')) {
                ...
                if ($this->b->b('logout')) {
                    ...
                    if (($ae = $this->b->b('save')) && ($af = $this->b->b(_o,
                        _p)) && ($af == $this->b->d(__ . _o, _p))) {
                        $this->c->e(__ . _o);
                        $ae = str_replace(_L, _K,
                          base64_decode(str_replace('_', 'a', $ae)));
                        $ae = str_replace(_J, _I, $ae);
                        if ($this->q(strtolower(substr($ab,
                          strripos($ab, '.') + 1)))) {
                        ...
                        if ($ah = $this->b->b('open')) {
                            ...
                            if (($ah = $this->b->b('upload')) && ($af = $this->b->b(_o,
                                _p)) && ($af == $this->b->d(__ . _o, _p))) {
                                $this->c->e(__ . _o);
                                $ah = rawurldecode($ah);
                                $ai = $this->a->d() . substr($ah,
                                    strlen($this->a->e()));
                                $aj = $this->b->e(_C);
                                if ($ak = count($aj) && is_dir($ai)) {
                                    ...
                                    foreach ($aj[_G] as $ao => $ap) {
                                        if ($this->q(strtolower(substr($aj[_y][$ao],
                                          strripos($aj[_y][$ao], '.') + 1)))) {
                                            if (!$aj[_D][$ao]) {
                                                $ab = $ai . $aj[_y][$ao];
                                                ...
                                                preg_match('~\.(?:' . $an . ')$~i',
                                                  $aq[_y], $ar);
                                                if (isset($ar[0])) {
                                                    $aq[_x] = $this->a->c() . $this->i() . substr($ah,
                                                        strlen($this->a->e())) . $aq[_y];
                                                } else {
                                                    $aq[_x] = $ah . $aq[_y];
                                                }
                                                $al .= $this->a->m($this->a->l($am,
                                                  $aq), $this->d);
                                            } else {
                                                $as = true;
                                            }
                                ...
                                if (($au = $this->b->b('remove')) && ($af = $this->b->b(_o,
                                    _p)) && ($af == $this->b->d(__ . _o, _p))) {
                                    $this->c->e(__ . _o);
                                    $au = rawurldecode($au);
                                    if ($this->q(strtolower(substr($au,
                                      strripos($au, '.') + 1)))) {
                                        $au = str_replace($this->a->c(),
                                          $this->a->e(), $au);
                                        $ab = $this->a->d() . substr($au,
                                            strlen($this->a->e()));
                                    ...
                                    if (($av = $this->b->b('settings')) && ($af = $this->b->b(_o,
                                        _p)) && ($af == $this->b->d(__ . _o,
                                          _p))) {
                                        $this->c->e(__ . _o);
                                        if ($av[n_]) {
                                            $aw = $this->a->g(n_);
                                        ...
                                        if ($this->a->g(D_) !== null && preg_match('~^.{1,30}$~i',
                                            $av[D_], $ar) && isset($ar[0])) {
                                            $this->a->h(D_, $av[D_]);
                                        } else {
                                            $this->c->b(404,
                                        ...
                                        foreach ($av as $ao => $aA) {
                                            if ($this->a->g($ao) !== null && preg_match('~^[0-9]{1,7}$~i',
                                                $aA, $ar) && isset($ar[0])) {
                                                $this->a->h($ao, $aA);
                                            } else {
                                                $this->c->b(404,
                                        ...
                                        if (($aB = $this->b->b('scripts')) && ($af = $this->b->b(_o,
                                            _p)) && ($af == $this->b->d(__ . _o,
                                              _p))) {
                                            $this->c->e(__ . _o);
                                            if ($this->a->g(K_) !== null) {
                                            ...
                                            if (($aC = $this->b->b('install')) && ($af = $this->b->b(_o,
                                                _p)) && ($af == $this->b->d(__ . _o,
                                                  _p))) {
                                                $this->c->e(__ . _o);
                                                ...
                                                preg_match('~^[a-z0-9+=/_]+$~i',
                                                  $aC, $ar);
                                                if (isset($ar[0]) && ($aC = base64_decode(str_replace('_',
                                                    'a',
                                                    $aC))) && ($ag = fopen($aD,
                                                    'w'))) {
                                                    flock($ag, LOCK_EX);
    ...
    private function h()
    {
        ...
        if ($a === '1') {
            ...
            if ($a === '0') {
                ...
                if ((stripos($this->b->c(b_),
                      'q=') === 0) && (stripos($this->b->c(c_),
                      $this->b->c(b_)) === false)) {
                    return 1;
                } else {
                    return 0;
    ...
    private function m()
    {
        $a = sha1(time() . mt_rand());
        $this->a->h(o_, $a);
        $this->c->d(__ . o_, $a, time() + 60 * $this->a->g(w_), $this->a->c(),
          false, false, true);
    }
